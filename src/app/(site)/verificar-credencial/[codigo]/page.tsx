...
import { getCredencialSolicitudByCodigo } from "@/lib/site-info";

export const metadata: Metadata = {
  title: "Verificación de credencial | CPT Santa Fe",
  description: "Verificá la autenticidad de una credencial digital emitida por el CPT Santa Fe.",
};
...
export default async function VerificarCredencialPage({ params }: { params: Promise<{ codigo: string }> }) {
  const { codigo } = await params;
  const solicitud = await getCredencialSolicitudByCodigo(codigo);
        ...
        <div className="rounded-2xl border border-surface-border bg-white p-8 text-center shadow-sm">
          {valido && solicitud ? (
            <>
              ...
              />
              <h1 className="mt-4 text-xl font-semibold text-ink-900">Credencial válida</h1>
              <p className="mt-1 text-sm text-ink-600">
                Esta credencial fue emitida por el Colegio Profesional de Maestros Mayores de Obras y Técnicos de
                Santa Fe.
              </p>
              ...
              <dl className="mt-6 space-y-3 rounded-xl bg-surface p-5 text-left text-sm">
                ...
                {solicitud.tituloProfesional && (
                  <div className="flex justify-between gap-4">
                    <dt className="text-ink-500">Tecnicatura</dt>
                    <dd className="font-semibold text-ink-900">{solicitud.tituloProfesional}</dd>
                  </div>
                )}
                ...
                <div className="flex justify-between gap-4">
                  <dt className="text-ink-500">Institución</dt>
                  <dd className="font-semibold text-ink-900">CPT Santa Fe — Distrito I</dd>
                </div>
                <div className="flex justify-between gap-4">
            ...
            <>
              ...
              <h1 className="mt-4 text-xl font-semibold text-ink-900">Credencial no válida</h1>
              <p className="mt-2 text-sm text-ink-600">
                No encontramos una credencial vigente con este identificador. Si creés que esto es un error,
